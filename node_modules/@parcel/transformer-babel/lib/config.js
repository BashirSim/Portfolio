...
async function load(config, options, logger) {
  ...
  let addIncludedFile = file => {
    if (JS_EXTNAME_RE.test(_path().default.extname(file))) {
      ...
      config.shouldInvalidateOnStartup(); // But also add the config as a dev dependency so we can at least attempt invalidation in watch mode.
  ...
  if (partialConfig.fileHandling != null && partialConfig.fileHandling !== 'transpile') {} else if (partialConfig.hasFilesystemConfig()) {
    ...
    if (hasRequire(partialConfig.options)) {
      ...
      config.setResultHash((0, _utils().hashObject)(partialConfig.options));
    }
  } else {
    await buildDefaultBabelConfig(options, config);
  }
...
async function buildDefaultBabelConfig(options, config) {
  ...
  babelOptions.presets = (babelOptions.presets || []).map(preset => internalBabelCore().createConfigItem(preset, {
    type: 'preset',
    dirname: BABEL_TRANSFORMER_DIR
  }));
  babelOptions.plugins = (babelOptions.plugins || []).map(plugin => internalBabelCore().createConfigItem(plugin, {
    type: 'plugin',
    dirname: BABEL_TRANSFORMER_DIR
  }));
  config.setResult({
